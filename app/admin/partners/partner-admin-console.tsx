...
export function PartnerAdminConsole() {
  ...
  async function provision(event: React.FormEvent<HTMLFormElement>) {
    ...
    setProvisioning(true); setError(""); setCreatedLink("")
  ...
  return <div className="space-y-8">
    ...
    <section className="glass-surface rounded-3xl p-6 md:p-8"><p className="text-xs font-medium uppercase tracking-[0.2em] text-primary">Provisioning</p><h2 className="mt-3 text-2xl font-semibold text-foreground">Create a partner firm</h2><p className="mt-2 text-sm text-muted-foreground">Creates the firm slug, sends the Supabase admin invite, and gives you the client-facing link.</p><form onSubmit={provision} className="mt-6 grid gap-4 md:grid-cols-2"><label className="grid gap-2 text-sm text-foreground">Firm name<input required value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value, slug: current.slug || slugify(event.target.value) }))} className="rounded-xl border border-border bg-background/60 px-4 py-3 outline-none focus:border-primary" /></label><label className="grid gap-2 text-sm text-foreground">Slug<input required pattern="[a-z0-9]+(-[a-z0-9]+)*" value={form.slug} onChange={(event) => setForm((current) => ({ ...current, slug: slugify(event.target.value) }))} className="rounded-xl border border-border bg-background/60 px-4 py-3 outline-none focus:border-primary" /></label><label className="grid gap-2 text-sm text-foreground">Admin email<input required type="email" value={form.adminEmail} onChange={(event) => setForm((current) => ({ ...current, adminEmail: event.target.value }))} className="rounded-xl border border-border bg-background/60 px-4 py-3 outline-none focus:border-primary" /></label><label className="grid gap-2 text-sm text-foreground">Logo URL <span className="text-xs text-muted-foreground">optional</span><input type="url" value={form.logoUrl} onChange={(event) => setForm((current) => ({ ...current, logoUrl: event.target.value }))} className="rounded-xl border border-border bg-background/60 px-4 py-3 outline-none focus:border-primary" /></label><label className="grid gap-2 text-sm text-foreground md:col-span-2">Notes <span className="text-xs text-muted-foreground">optional</span><textarea value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} className="min-h-24 rounded-xl border border-border bg-background/60 px-4 py-3 outline-none focus:border-primary" /></label><div className="md:col-span-2"><Button type="submit" disabled={provisioning} className="rounded-xl">{provisioning ? "Provisioning…" : "Provision firm"}<ArrowUpRight className="h-4 w-4" /></Button></div></form>{createdLink ? <div className="mt-6 rounded-2xl border border-primary/30 bg-primary/5 p-4"><p className="text-xs font-medium uppercase tracking-wider text-primary">Firm link ready</p><div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center"><code className="flex-1 select-all break-all rounded-lg bg-background/60 px-3 py-2 text-sm text-foreground">{createdLink}</code><Button type="button" variant="outline" onClick={() => void copyLink()} className="rounded-xl">{copied ? <Check className="h-4 w-4" /> : <Clipboard className="h-4 w-4" />}{copied ? "Copied" : "Copy link"}</Button></div><p className="mt-3 text-xs text-muted-foreground">The Supabase admin invitation was sent to the address provided.</p></div> : null}</section>
