...
async function adminForToken(req: NextRequest) {
  const token = req.headers.get("authorization")?.replace(/^Bearer\s+/i, "").trim()
  if (!token) return null
  const { data, error } = await supabaseAdmin.auth.getUser(token)
  if (error || !data.user) return null
  const { data: admin } = await supabaseAdmin.from("firm_admins").select("firm_id").eq("user_id", data.user.id).maybeSingle()
  return admin ? { user: data.user, firmId: admin.firm_id } : null
}

export async function GET(req: NextRequest) {
  const auth = await adminForToken(req)
