...
export function FirmIntake({ slug, firmName }: { slug: string; firmName: string }) {
  ...
  const [status, setStatus] = useState<"idle" | "working" | "success" | "full" | "error">("idle")
  ...
  useEffect(() => {
    ...
    void supabase.auth.getSession().then(async ({ data }) => {
      ...
      if (!active) return
      setStatus(result.ok ? "success" : result.code === "seats_full" ? "full" : "error")
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!enrollment.ok) {
      setStatus(enrollment.code === "seats_full" ? "full" : "error")
