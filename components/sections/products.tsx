...
function StatusBadge({ status }: { status: StatusType }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-xs font-medium ${styles[status]}`}>
      ...
      `}</style>
      <span className={`h-1.5 w-1.5 rounded-full ${dotColors[status]} ${status === "live" ? "dot-live" : "dot-dev"}`} />
      {labels[status]}
    </span>
  )
}
...
function HooperIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <style>{`
        @keyframes hoop-ellipse { 0%,100%{opacity:0.3} 50%{opacity:1} }
...
const products: ProductCardProps[] = [
  {
    name: "Smart Storage",
    description: "Upload your receipts and invoices. AI reads and organizes them. Reports ready. Turn your files into tax-ready summaries — export-ready for QuickBooks & Xero. Now works inside Claude — run your tax bundle and pull a QuickBooks export right from chat.",
