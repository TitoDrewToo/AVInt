...
export function DataModelView({ files, folders, onManualEntry, onReclassify, onRetry, onOpenSource }: {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background">
    ...
    <main className="min-h-0 flex-1 overflow-auto p-4">
      ...
      </div> : <SchemaView catalog={model?.catalog ?? []} datasets={model?.datasets ?? []} columns={model?.datasetColumns ?? []} mappingProfiles={model?.mappingProfiles ?? []} query={query} />}
    </main>
...
function SchemaView({ catalog, datasets, columns, mappingProfiles, query }: { catalog: CatalogField[]; datasets: Dataset[]; columns: DatasetColumn[]; mappingProfiles: MappingProfile[]; query: string }) {
