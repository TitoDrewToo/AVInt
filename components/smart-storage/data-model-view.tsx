...
type MappingRule = { sourceField: string; targetField: string; coercion: string } | { targetField: string; targetType: string; candidates: Array<{ sourceField: string; coercion: string }>; required: boolean; onMissing: string; onConflict: string; role?: "time" | "currency" }
type MappingProfile = { slug: string; title: string; description: string | null; status: "draft" | "active"; version: number; previewed_version: number | null; preview_summary?: { mode?: string; activationReady?: boolean; sourceDatasets?: number; includedDatasets?: number; outputRows?: number; conflicts?: number; typeFailures?: number } | null; mappings: MappingRule[] }
type DataRelationship = { slug: string; title: string; description: string | null; status: "draft" | "active"; version: number; previewed_version: number | null; definition: { leftVirtualDatasetSlug: string; rightVirtualDatasetSlug: string; leftKey: string; rightKey: string; cardinality: string }; preview_summary: { matchRate?: number; projectedRows?: number } | null }
type ModelPayload = {
...
export function DataModelView({ files, folders, onManualEntry, onReclassify, onRetry, onOpenSource }: {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background">
    ...
    <main className="min-h-0 flex-1 overflow-auto p-4">
      ...
      </div> : <SchemaView catalog={model?.catalog ?? []} datasets={model?.datasets ?? []} columns={model?.datasetColumns ?? []} mappingProfiles={model?.mappingProfiles ?? []} relationships={model?.relationships ?? []} query={query} />}
    </main>
...
function SchemaView({ catalog, datasets, columns, mappingProfiles, relationships, query }: { catalog: CatalogField[]; datasets: Dataset[]; columns: DatasetColumn[]; mappingProfiles: MappingProfile[]; relationships: DataRelationship[]; query: string }) {
  ...
  return <div className="space-y-5"><section><h3 className="mb-2 text-xs font-semibold uppercase tracking-wider">Canonical record fields</h3><div className="overflow-hidden rounded-xl border border-border"><div className="grid grid-cols-[1fr_120px_80px] bg-muted/40 px-3 py-2 text-[10px] uppercase text-muted-foreground"><span>Field</span><span>Type</span><span>Seen</span></div>{visibleCatalog.map((field) => <div key={field.field_key} className="grid grid-cols-[1fr_120px_80px] border-t border-border px-3 py-2 text-xs"><span><code>{field.field_key}</code>{field.is_custom && <span className="ml-2 text-[10px] text-primary">Custom</span>}</span><span>{field.value_types.join(", ")}</span><span>{field.occurrence_count}</span></div>)}</div></section><section><h3 className="mb-2 text-xs font-semibold uppercase tracking-wider">Dataset schemas</h3><div className="overflow-hidden rounded-xl border border-border"><div className="grid grid-cols-[1fr_1fr_120px_90px] bg-muted/40 px-3 py-2 text-[10px] uppercase text-muted-foreground"><span>Dataset</span><span>Column key</span><span>Type / role</span><span>Review</span></div>{visibleColumns.map((column) => <div key={column.id} className="grid grid-cols-[1fr_1fr_120px_90px] border-t border-border px-3 py-2 text-xs"><span>{datasetById.get(column.dataset_id)?.name ?? "Dataset"}</span><span><code>{column.key}</code><span className="ml-2 text-muted-foreground">{column.label}</span></span><span>{column.data_type}{column.role ? ` · ${column.role}` : ""}</span><span>{column.needs_review ? "Required" : "Ready"}</span></div>)}</div></section><section><h3 className="mb-2 text-xs font-semibold uppercase tracking-wider">Mapping profiles</h3><div className="space-y-2">{mappingProfiles.length ? mappingProfiles.map((profile) => <div key={profile.slug} className="rounded-xl border border-border p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-medium">{profile.title}</p><code className="text-[10px] text-muted-foreground">{profile.slug} · v{profile.version}</code></div><span className={`rounded-full px-2 py-1 text-[10px] ${profile.status === "active" ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground"}`}>{profile.status}</span></div><div className="mt-2 flex flex-wrap gap-1.5">{profile.mappings.map((rule) => <MappingRuleChip key={`${profile.slug}:${rule.targetField}`} rule={rule} />)}</div>{profile.preview_summary?.mode === "reconciliation" && <p className="mt-2 text-[10px] text-muted-foreground">Preview: {profile.preview_summary.includedDatasets ?? 0}/{profile.preview_summary.sourceDatasets ?? 0} datasets · {profile.preview_summary.outputRows ?? 0} rows · {profile.preview_summary.conflicts ?? 0} conflicts · {profile.preview_summary.typeFailures ?? 0} type failures{profile.preview_summary.activationReady === false ? " · not ready" : ""}</p>}</div>) : <p className="rounded-xl border border-dashed border-border p-3 text-xs text-muted-foreground">No mapping profiles saved yet. Connected agents can draft, preview, and activate them through Smart Storage MCP.</p>}</div></section><section><h3 className="mb-2 text-xs font-semibold uppercase tracking-wider">Virtual dataset relationships</h3><div className="space-y-2">{relationships.length ? relationships.map((relationship) => <div key={relationship.slug} className="rounded-xl border border-border p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-medium">{relationship.title}</p><code className="text-[10px] text-muted-foreground">{relationship.slug} · v{relationship.version}</code></div><span className={`rounded-full px-2 py-1 text-[10px] ${relationship.status === "active" ? "bg-emerald-500/10 text-emerald-600" : "bg-muted text-muted-foreground"}`}>{relationship.status}</span></div><p className="mt-2 text-[11px] text-muted-foreground"><code>{relationship.definition.leftVirtualDatasetSlug}.{relationship.definition.leftKey}</code> = <code>{relationship.definition.rightVirtualDatasetSlug}.{relationship.definition.rightKey}</code> · {relationship.definition.cardinality.replaceAll("_", " ")}</p>{relationship.preview_summary && <p className="mt-1 text-[10px] text-muted-foreground">Preview: {Math.round((relationship.preview_summary.matchRate ?? 0) * 100)}% matched · {relationship.preview_summary.projectedRows ?? 0} rows</p>}</div>) : <p className="rounded-xl border border-dashed border-border p-3 text-xs text-muted-foreground">No relationships saved yet. Connected agents can draft, preview, and activate equality joins through Smart Storage MCP.</p>}</div></section></div>
}

function MappingRuleChip({ rule }: { rule: MappingRule }) {
  if ("candidates" in rule) return <span className="rounded border border-border px-1.5 py-1 text-[10px]"><code>{rule.candidates.map((candidate) => candidate.sourceField).join(" | ")}</code> → <code>{rule.targetField}</code> · {rule.targetType}{rule.role ? `/${rule.role}` : ""} · {rule.required ? "required" : rule.onMissing}</span>
  return <span className="rounded border border-border px-1.5 py-1 text-[10px]"><code>{rule.sourceField}</code> → <code>{rule.targetField}</code> · {rule.coercion}</span>
}
