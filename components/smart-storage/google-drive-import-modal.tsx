  ...
  return <>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="max-w-lg" aria-describedby="google-drive-import-description">
        ...
        {!connected ? <div className="rounded-xl border border-border bg-muted/30 p-5"><p className="text-sm text-foreground">Connect the Google account that contains the documents you want to process.</p><Button type="button" className="mt-4" onClick={() => void connect()} disabled={busy}>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <HardDrive className="h-4 w-4" />} Connect Google Drive</Button></div> : <div className="rounded-xl border border-primary/25 bg-primary/5 p-5"><p className="flex items-center gap-2 text-sm font-medium text-foreground"><Check className="h-4 w-4 text-primary" /> Google Drive connected</p><p className="mt-2 text-sm text-muted-foreground">Open Google’s native picker, open the folder you want, then select the files inside it for import.</p><div className="mt-4 flex flex-wrap gap-2"><Button type="button" onClick={() => void openPicker()} disabled={busy}>{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <HardDrive className="h-4 w-4" />} Open Google Drive picker</Button><Button type="button" variant="outline" onClick={() => void connect()} disabled={busy}>Reconnect Google Drive</Button></div>{selectedCount > 0 && <p className="mt-3 text-xs text-muted-foreground">{selectedCount} file{selectedCount === 1 ? "" : "s"} selected for import.</p>}</div>}
      </DialogContent>
    </Dialog>
  </>
}
