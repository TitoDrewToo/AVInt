...
export function SavedReportManager({ folderId }: { folderId: string | null }) {
  ...
  return <section className="space-y-3 border-b border-border p-3">
    ...
    <div className="flex gap-1"><input value={assistantPrompt} onChange={(event) => setAssistantPrompt(event.target.value)} placeholder="Ask the assistant for a report…" className="min-w-0 flex-1 rounded border border-border bg-background px-2 py-1.5 text-xs" /><button disabled={busy || !assistantPrompt.trim()} onClick={() => void propose()} className="rounded border border-primary/30 px-2 text-primary disabled:opacity-40" title="Create a proposal"><Sparkles className="h-3.5 w-3.5" /></button></div>
    {error && <p className="rounded border border-destructive/30 bg-destructive/5 px-2 py-1.5 text-xs text-destructive">{error}</p>}
    {editor && <div className="space-y-2 rounded-lg border border-primary/30 bg-background p-2"><div className="flex items-center justify-between"><p className="text-[10px] font-medium uppercase tracking-wider">{editingSlug ? `Edit ${editingSlug}` : "Review definition before saving"}</p><button onClick={() => setEditor(null)}><X className="h-3.5 w-3.5" /></button></div><textarea value={editor} onChange={(event) => setEditor(event.target.value)} className="h-64 w-full resize-y rounded border border-border bg-muted/20 p-2 font-mono text-[10px] outline-none" spellCheck={false} /><button disabled={busy} onClick={() => void save()} className="w-full rounded bg-primary px-2 py-1.5 text-xs text-primary-foreground">{busy ? "Validating…" : "Confirm and save"}</button></div>}
    <div className="max-h-44 space-y-1 overflow-y-auto">{definitions.map((definition) => <div key={definition.slug} className={`rounded-lg border p-2 ${selected?.slug === definition.slug ? "border-primary/50 bg-primary/5" : "border-border"}`}><button onClick={() => void run(definition)} className="w-full text-left"><p className="truncate text-xs font-medium">{definition.title}</p><p className="text-[10px] text-muted-foreground">{definition.slug} · v{definition.version} · {definition.authored_by}</p></button><div className="mt-2 flex gap-2 text-muted-foreground"><button title="Run" onClick={() => void run(definition)}><Play className="h-3 w-3" /></button><button title="Edit" onClick={() => void edit(definition)}><Pencil className="h-3 w-3" /></button><button title="Duplicate" onClick={() => void duplicate(definition)}><Copy className="h-3 w-3" /></button><button title="Archive" onClick={() => void archive(definition)}><Trash2 className="h-3 w-3" /></button></div></div>)}</div>
