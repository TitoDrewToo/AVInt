...
export function StudioInquiryForm() {
  ...
  return <form className="glass-surface rounded-3xl p-6 text-left md:p-10" onSubmit={submit} noValidate>
    ...
    <div className="grid gap-5 md:grid-cols-2">
      ...
      <label className="grid gap-2 text-sm text-foreground">Email<input required name="email" type="email" autoComplete="email" value={values.email} onChange={(event) => update("email", event.target.value)} className="rounded-xl border border-border/70 bg-background/60 px-4 py-3 text-foreground outline-none transition-colors placeholder:text-muted-foreground focus:border-primary" /></label>
    ...
    <label className="mt-5 grid gap-2 text-sm text-foreground">What would you like to build? <span className="text-xs text-muted-foreground">required</span><textarea required aria-required="true" name="message" value={values.message} onChange={(event) => update("message", event.target.value)} className="min-h-36 resize-y rounded-xl border border-border/70 bg-background/60 px-4 py-3 text-foreground outline-none transition-colors placeholder:text-muted-foreground focus:border-primary" /></label>
