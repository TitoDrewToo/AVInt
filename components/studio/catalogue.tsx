...
function CatalogueBody({ section }: { section?: string }) {
  ...
  return <div data-motion={motion ? "on" : "off"} className="space-y-8">
    ...
    <div className="sticky top-4 z-20 flex flex-wrap items-center justify-between gap-4 border border-[var(--line)] bg-[var(--surface)]/95 p-3 backdrop-blur"><StudioPaletteSwitcher /><StudioMotionToggle /><nav aria-label="Studio library sections" className="flex flex-wrap gap-2">{studioSections.map((item) => <a key={item.id} href={`#studio-section-${item.id}`} className="font-[var(--font-mono)] text-[10px] uppercase tracking-[0.1em] text-[var(--text-dim)] hover:text-[var(--text)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[var(--brand)]">{item.label}</a>)}</nav></div>
    {sections.map((item) => <section key={item.id} id={`studio-section-${item.id}`} aria-labelledby={`studio-${item.id}`} className="scroll-mt-28"><div className="mb-5"><p className="font-[var(--font-mono)] text-[10px] uppercase tracking-[0.16em] text-[var(--brand)]">Library section</p><h2 id={`studio-${item.id}`} className="mt-2 font-[var(--font-display)] text-3xl font-bold text-[var(--text)]">{item.label}</h2><p className="mt-2 text-sm text-[var(--text-dim)]">{item.description}</p></div><div className="grid gap-5 md:grid-cols-2">{getStudioAssets(item.id).map((asset) => <StudioCard key={asset.id} asset={asset} />)}</div></section>)}
  </div>
}
