...
import { ArrowUpRight, Activity, GitCommitHorizontal } from "lucide-react"
import { Footer } from "@/components/footer"
import { Navbar } from "@/components/navbar"

export function OperationsShell({ children, active }: { children: React.ReactNode; active: "changelog" | "status" }) {
  return (
      ...
      <main className="relative z-[1] flex-1 overflow-hidden px-4 py-10 md:px-8 md:py-14">
        ...
        <div className="relative mx-auto max-w-6xl">
          <header className="mb-10 flex flex-col justify-between gap-8 md:flex-row md:items-end">
            ...
            <nav aria-label="Systems operations" className="flex shrink-0 gap-2 rounded-2xl border border-border bg-background/50 p-1.5">
              <Link href="/systems/status" className={`cw-button-flow inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-medium transition-colors ${active === "status" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-primary"}`}><Activity className="h-3.5 w-3.5" />Status</Link>
              <Link href="/systems/changelog" className={`cw-button-flow inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs font-medium transition-colors ${active === "changelog" ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-primary"}`}><GitCommitHorizontal className="h-3.5 w-3.5" />Changelog</Link>
            </nav>
          </header>
          {children}
