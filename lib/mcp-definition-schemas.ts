...
const metric = z.object({ aggregation: z.enum(["count", "count_distinct", "sum", "average", "min", "max", "ratio"]), field: field.optional(), numerator: field.optional(), denominator: field.optional(), onZero: z.enum(["suppress", "null"]).optional(), role: z.enum(["time", "currency", "identifier", "dimension", "descriptor", "ignored", "measure_additive", "measure_semi_additive", "measure_non_additive"]).optional() }).strict().describe("ratio uses numerator and denominator fields; count may omit field. Other aggregations require field. role is an optional semantic override.")
...
export const mappingProfileSchema = z.object({ title, description, source: materializedSourceSchema, scope, mappings: z.array(z.union([
  ...
  z.object({ targetField: field, targetType: z.enum(["text", "number", "date", "boolean"]), candidates: z.array(z.object({ sourceField: field, coercion }).strict()).min(1).max(10), required: z.boolean(), expected: z.boolean().optional(), onMissing: z.enum(["null", "exclude_row", "exclude_dataset", "reject"]), onConflict: z.enum(["reject", "first_non_empty"]), role: z.enum(["time", "currency", "identifier", "dimension", "descriptor", "ignored", "measure_additive", "measure_semi_additive", "measure_non_additive"]).optional() }).strict(),
])).min(1).max(50) }).strict()
