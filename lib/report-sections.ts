...
export async function getExpenseSummary(userId: string, _context: Context, filters: Filters): Promise<{ expenses: unknown[] }> {
  ...
  let query = scopedDb(userId).from("records").select("id, file_id, document_type, occurred_on, amount, currency, category, confidence, files!inner(filename, document_type)").in("file_id", fileIds).is("parent_record_id", null).is("excluded_at", null).order("occurred_on", { ascending: false })
  ...
  const records = data ?? []
  const attributes = records.length === 0 ? [] : await readComplete((from, to) => scopedDb(userId).from("record_attributes").select("record_id, value, records!inner(file_id)", { count: "exact" }).in("records.file_id", fileIds).eq("field_key", "vendor_name").range(from, to), 100_000, "report_section_expense_attributes")
...
export async function getIncomeSummary(userId: string, context: Context, filters: Filters): Promise<{ income: unknown[] }> {
  ...
  let query = scopedDb(userId).from("records").select("id, file_id, document_type, occurred_on, amount, currency, direction, confidence, files!inner(filename, document_type)").in("file_id", fileIds).is("parent_record_id", null).is("excluded_at", null).order("occurred_on", { ascending: false })
  if (filters.dateFrom) query = query.gte("occurred_on", filters.dateFrom)
  if (filters.dateTo) query = query.lte("occurred_on", filters.dateTo)
  const { data, error } = await query
  if (error) throw new Error(error.message)
  const records = data ?? []
  const attributes = records.length === 0 ? [] : await readComplete((from, to) => scopedDb(userId).from("record_attributes").select("record_id, field_key, value, records!inner(file_id)", { count: "exact" }).in("records.file_id", fileIds).in("field_key", ["employer_name", "gross_income", "net_income", "income_source"]).range(from, to), 100_000, "report_section_income_attributes")
...
export async function getProfitLoss(userId: string, context: Context, filters: Filters): Promise<{ incomeRows: unknown[]; expenseRows: unknown[] }> {
  const fileIds = await context.fileIds()
  {
    let query = scopedDb(userId).from("records").select("id, file_id, document_type, occurred_on, amount, currency, direction, category, files!inner(document_type)").in("file_id", fileIds).is("parent_record_id", null).is("excluded_at", null).order("occurred_on", { ascending: true })
    ...
    const attributes = records.length === 0 ? [] : await readComplete((from, to) => scopedDb(userId).from("record_attributes").select("record_id, field_key, value, records!inner(file_id)", { count: "exact" }).in("records.file_id", fileIds).in("field_key", ["employer_name", "gross_income", "net_income", "income_source", "vendor_name"]).range(from, to), 100_000, "report_section_profit_loss_attributes")
...
export async function getContractSummary(userId: string, context: Context, filters: Filters): Promise<{ contracts: unknown[]; obligations: Record<string, unknown[]> }> {
  ...
  const { data: records, error: contractErr } = await scopedDb(userId).from("records").select("id, file_id, occurred_on, period_start, period_end, amount, currency, confidence, files!inner(filename, document_type)").in("file_id", fileIds).is("parent_record_id", null).is("excluded_at", null).order("occurred_on", { ascending: false })
  if (contractErr) throw new Error(contractErr.message)
  const attributes = (records ?? []).length === 0 ? [] : await readComplete((from, to) => scopedDb(userId).from("record_attributes").select("record_id, field_key, value, records!inner(file_id)", { count: "exact" }).in("records.file_id", fileIds).in("field_key", ["counterparty_name", "invoice_number", "payment_method"]).range(from, to), 100_000, "report_section_contract_attributes"); const byRecord = new Map<string, Map<string, unknown>>(); for (const attribute of attributes ?? []) { const fields = byRecord.get(attribute.record_id) ?? new Map<string, unknown>(); fields.set(attribute.field_key, attribute.value); byRecord.set(attribute.record_id, fields) }
  ...
  const { data: obligs, error: obligErr } = await scopedDb(userId).from("payment_obligations").select("*").in("file_id", visibleFileIds).order("due_date", { ascending: true }); if (obligErr) throw new Error(obligErr.message)
...
export async function getKeyTerms(userId: string, context: Context, filters: Filters): Promise<{ docs: unknown[] }> {
  ...
  const { data: parents, error } = await scopedDb(userId).from("records").select("id, file_id, occurred_on, period_start, period_end, amount, currency, confidence, files!inner(filename, document_type)").in("file_id", fileIds).is("parent_record_id", null).is("excluded_at", null).order("occurred_on", { ascending: false }); if (error) throw new Error(error.message)
  const parentRows = parents ?? []; const { data: children, error: childrenError } = parentRows.length === 0 ? { data: [], error: null } : await scopedDb(userId).from("records").select("id, parent_record_id, source_key, amount").in("file_id", fileIds).not("parent_record_id", "is", null).is("excluded_at", null).order("source_key", { ascending: true }); if (childrenError) throw new Error(childrenError.message)
  const attributes = parentRows.length === 0 ? [] : await readComplete((from, to) => scopedDb(userId).from("record_attributes").select("record_id, field_key, value, records!inner(file_id)", { count: "exact" }).in("records.file_id", fileIds).range(from, to), 100_000, "report_section_key_term_attributes")
