...
export async function fetchSmartStorageFilesPage(userId: string, offset = 0, limit = SMART_STORAGE_PAGE_SIZE): Promise<SmartStorageFilesPage> {
  const { data, error } = await supabase
    .from("files")
    .select("id, filename, file_type, file_size, document_type, created_at, storage_path, folder_id, upload_batch_id, upload_status, scan_reason, analysis_json, analyzed_at")
    .eq("user_id", userId)
    .order("created_at", { ascending: false })
