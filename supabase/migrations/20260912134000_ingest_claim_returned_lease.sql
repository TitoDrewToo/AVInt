-- Data-modifying CTE readers see the pre-update table snapshot. Return new
-- status and lease from RETURNING, not the old base-table columns.
...
begin
  ...
  with newly_claimed as (
    ...
    returning i.id, i.status, i.lease_token
  )
  select
    i.batch_id,
    i.id,
    i.item_index,
    coalesce(c.status, i.status),
    i.file_id,
    coalesce(c.lease_token, i.lease_token),
    c.id is not null
  from public.ingest_batch_items i
